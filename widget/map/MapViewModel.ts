...
async function addScaleBar(widget: ScaleBarWidget, view: MapView): Promise<ScaleBar|null> {
    return new Promise(resolve => {
        ...
        returnConfig(configFile, null).then(config => {
            ...
            const _style = getWidgetConfigKeyValue(config as ScaleBarWidget, "style", widget.unit? widget.unit: "line") as "ruler"|"line";
...
function getWidgetConfigKeyValue(widget: MapWidget, configKey: string, defaultValue=null as unknown) {
...
function getWidgetLocaleConfigKeyValue(widgetLocale: MapWidgetLocale, configKey: string, defaultValue=null as unknown) {
    let result = defaultValue;
    if (widgetLocale) {
...
async function mwAsyncForEach(array: Array<MapWidget>, callback: { (widget: MapWidget): Promise<void>; (arg0: MapWidget, arg1: number, arg2: MapWidget[]): unknown; }) {
